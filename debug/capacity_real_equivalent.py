...
def test_shape(input_size, output_size, shape_raw, classifier=True, capacity_equivalent=True):
    ...
    shape = [ComplexDense(input_size=input_size, output_size=shape_raw[0] if len(shape_raw) != 0 else output_size,
                          input_dtype=np.complex64)]
    for i in range(1, len(shape_raw)):
        shape.append(ComplexDense(output_size=shape_raw[i], dropout=None))
    if len(shape_raw) != 0:
