...
def _downsample_cvnn(inputs, units):
    c0 = layers.ComplexConv2D(units, activation='relu', kernel_size=3, dtype=tf.float32)(inputs)
    c1 = layers.ComplexConv2D(units, activation='relu', kernel_size=3, dtype=tf.float32)(c0)
    c2 = layers.ComplexMaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='valid', dtype=tf.float32)(c1)
    return c0, c1, c2
...
def _upsample_cvnn(in1, in2, units, crop):
    t01 = layers.ComplexConv2DTranspose(units, kernel_size=2, strides=(2, 2), activation='relu')(in1)
    crop01 = tf.keras.layers.Cropping2D(cropping=(crop, crop))(in2)

    concat01 = tf.keras.layers.concatenate([t01, crop01], axis=-1)

    out1 = layers.ComplexConv2D(units, activation='relu', kernel_size=3)(concat01)
    out2 = layers.ComplexConv2D(units, activation='relu', kernel_size=3)(out1)
    return out1, out2
...
def get_cvnn_model():
    ...
    inputs = layers.ComplexInput(shape=INPUT_SIZE + (3,))
    ...
    c14, c15 = _upsample_cvnn(c13, c10, 512, 4)
    c16, c17 = _upsample_cvnn(c15, c7, 256, 16)
    c18, c19 = _upsample_cvnn(c17, c4, 128, 40)
    c20, c21 = _upsample_cvnn(c19, c1, 64, 88)
