        ...
        for it in range(iterations):
            ...
            for i, model in enumerate(self.models):
                if model.is_complex():
                    ...
                    x_val = x_test_real
                # model.reset_model()
                model.fit(x_train_iter, y_train, x_test=x_val, y_test=y_test,
                          learning_rate=learning_rate, epochs=epochs, batch_size=batch_size,
                          verbose=debug, fast_mode=not debug, save_to_file=False)
                train_loss, train_acc = model.evaluate(x_train_iter, y_train)
                test_loss, test_acc = model.evaluate(x_val, y_test)
                # save result
